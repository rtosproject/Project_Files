...
int main( int argc, char *argv[] ) {
  ...
  if( argc != 6 ) { // to change when adding priority, nice, algorithm, etc.
    ...
    printf( "RR: %i\n", SCHED_RR );
      ...
      // lower priority numbers are given priority
    
    return EXIT_FAILURE;
  }
  
  
  // set the scheduling policy & prio
	int policy = atoi( argv[ 4 ] );
	struct sched_param param;
	ZERO( param );
	param.sched_priority = atoi( argv[ 5 ] ); // set priority
	sched_setscheduler( 0 /* this process */, policy, &param );
